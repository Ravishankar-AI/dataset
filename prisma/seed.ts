...
import thumbnailObjectKeys from "./data/thumbnail-object-keys.json";

// Episodes whose first-frame JPEG was successfully generated by the one-off
// local script (generate-thumbnails.py) and uploaded to thumbnails/ --
// keyed by objectKey (the episode's parquet key), which is unique per
// episode and identical between this file and manual-seed.sql.
const EPISODES_WITH_THUMBNAILS = new Set<string>(thumbnailObjectKeys);
...
async function main() {
  ...
  for (const e of CLUTTER_SORT_EPISODES) {
    const objectKey = episodeParquetKey(clutterSort, e.episodeIndex);
    await prisma.episode.create({
      data: {
        ...
        objectKey,
        status: "cataloged",
        hasThumbnail: EPISODES_WITH_THUMBNAILS.has(objectKey),
      },
    });
  }
  ...
  for (const t of teleopTasks) {
    ...
    for (let episodeIndex = 0; episodeIndex < t.sampleCount; episodeIndex++) {
      ...
      const objectKey = episodeParquetKey(task, episodeIndex);
      await prisma.episode.upsert({
        where: { taskId_episodeIndex: { taskId: task.id, episodeIndex } },
        // Retroactively fixes durationSeconds/hasThumbnail on a row seeded
        // before these existed -- everything else on an existing row is
        // left alone.
        update: { durationSeconds, hasThumbnail: EPISODES_WITH_THUMBNAILS.has(objectKey) },
        create: {
          datasetId: teleopCapture.id,
          taskId: task.id,
          episodeIndex,
